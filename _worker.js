...
const LEAD_OPERATIONS_BUILD = '408-LEAD-OPS-1.3';
...
function coverageFitLeadPayload(fields) {
  const allowed = [
    ...
    ['housing_context',40],['review_track',40],['product_track',40],['review_reason',60],['review_context',240],
    ['closing_date',40],['property_zip',10],
...
function formspreeLeadEndpoint(env) {
  const endpoint = String((env && env.FORMSPREE_ENDPOINT) || DEFAULT_FORMSPREE_ENDPOINT).trim();
  return /^https:\/\/formspree\.io\/f\/[A-Za-z0-9_-]+$/.test(endpoint) ? endpoint : '';
}

async function deliverFormspreeLead(fields, env) {
  const endpoint = formspreeLeadEndpoint(env);
  if (!endpoint) {
    const error = new Error('formspree_not_configured');
    error.status = 503;
    throw error;
  }
  ...
  // referrer so the server-side relay behaves like the production form.
  ...
  try {
    ...
    const error = new Error('formspree_unreachable');
    error.status = 502;
    throw error;
  }

  if (!upstream.ok) {
    const error = new Error('formspree_rejected');
...
async function handleLeadProxy(request, env) {
  ...
  // Formspree is also required for Dylan's immediate lead notification.
  let durable = null;
  try {
    durable = await deliverCoverageFitLead(fields, env);
  } catch (_) {}

  if (durable) {
    try {
      await deliverFormspreeLead(fields, env);
    } catch (notificationError) {
      // Returning a non-2xx response activates the browser-direct Formspree
      // fallback. The D1 write is already safe and idempotent.
      return jsonResponse(notificationError.status || 502, {
        ok:false,
        error:'formspree_notification_failed',
        delivery:'coveragefit_d1',
        durable:true,
        notification:'pending_browser_fallback',
        build:LEAD_OPERATIONS_BUILD,
        checkpoint_id:durable.checkpointId,
        crm_state:durable.crm?.state || 'pending'
      });
    }
    return jsonResponse(200, {
      ok:true, delivery:'coveragefit_d1_and_formspree', durable:true, notification:'sent', build:LEAD_OPERATIONS_BUILD,
      checkpoint_id:durable.checkpointId, crm_state:durable.crm?.state || 'pending'
    });
  }

  // If durable intake is unavailable, retain Formspree as an independent lead
  // fallback so the prospect is not silently lost.
  try {
    await deliverFormspreeLead(fields, env);
  } catch (error) {
    return jsonResponse(error.status || 502, { ok:false, error:'lead_delivery_failed' });
  }
...
function pageAssetRoute(pathname) {
  ...
  // The old public name over-promised a score before any policy evidence was
  // collected. Preserve bookmarks while making /snapshot/ the canonical path.
  if (path === '/score' || path === '/score/' || path === '/score/index.html') {
    return { redirect: '/snapshot/', status: 301 };
  }
  ...
  const canonicalDirectories = new Set(['/home','/contact','/buyer','/life','/life-ops','/neighbor','/snapshot','/auto-bundle','/healthcare','/teachers','/tech','/engineers','/local']);
  ...
  // Canonical routes such as /home/, /contact/, /buyer/, /life/, /snapshot/
