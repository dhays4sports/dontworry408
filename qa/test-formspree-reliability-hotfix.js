...
check('submission exposes browser-direct Formspree fallback', script.includes('await postLead(directEndpoint)'));
check('submission surfaces delivery failure instead of silently continuing', script.includes('We could not confirm that your request was saved'));
const worker=read('_worker.js');
check('worker sends Formspree after durable CoverageFit intake', worker.includes("delivery:'coveragefit_d1_and_formspree'") && worker.includes("notification:'sent'"));
check('worker failure activates browser-direct Formspree fallback', worker.includes("notification:'pending_browser_fallback'"));
