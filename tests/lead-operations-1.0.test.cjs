...
test('same-origin relay writes durably and sends the Formspree notification without PII in URLs', async () => {
  const original = global.fetch, calls=[];
  global.fetch = async (url,init) => {
    calls.push({url:String(url),init});
    return String(url).includes('coveragefit.com')
      ? Response.json({ok:true,durable:true,checkpointId:'408d_1234567890abcdef1234567890abcdef',crm:{state:'pending'}},{status:201})
  ...
  try {
    ...
    assert.equal(response.status,200); assert.equal(body.delivery,'coveragefit_d1_and_formspree'); assert.equal(body.durable,true); assert.equal(body.notification,'sent');
    assert.equal(calls.length,2); assert.equal(calls[0].url,'https://coveragefit.com/api/lead/intake'); assert.equal(calls[1].url,'https://formspree.io/f/mojgnegn');
    assert.equal(calls[0].url.includes('Maya'),false); assert.equal(calls[1].url.includes('Maya'),false); assert.ok(calls[0].init.headers['X-CoverageFit-Signature']);
    ...
    assert.match(String(calls[1].init.body),/first_name=Maya/);
  } finally { global.fetch = original; }
});

test('durable success plus Formspree failure activates the browser notification fallback', async () => {
  const original = global.fetch, calls=[];
  global.fetch = async (url,init) => {
    calls.push(String(url));
    if (String(url).includes('coveragefit.com')) return Response.json({ok:true,durable:true,checkpointId:'408d_1234567890abcdef1234567890abcdef',crm:{state:'pending'}},{status:201});
    return Response.json({ok:false},{status:503});
  };
  try {
    const worker = (await import(`${workerPath}?notificationFallback=${Date.now()}`)).default;
    const response = await worker.fetch(request(),{ COVERAGEFIT_LEAD_SYNC_SECRET:'test-secret-that-is-longer-than-thirty-two-characters', ASSETS:{fetch:original} });
    const body = await response.json();
    assert.equal(response.status,503);
    assert.equal(body.durable,true);
    assert.equal(body.notification,'pending_browser_fallback');
...
test('marketing-enabled lead forms expose separate optional automated marketing SMS consent', () => {
  const routes=['home','buyer','engineers','auto-bundle','teachers','healthcare'];
  ...
  const tech=fs.readFileSync(path.resolve(__dirname,'../tech/index.html'),'utf8');
  assert.doesNotMatch(tech,/name="automated_marketing_sms_consent"/);
  assert.match(tech,/This does not permit automated marketing texts/);
  assert.match(tech,/name="consent"[^>]*required/);
});
