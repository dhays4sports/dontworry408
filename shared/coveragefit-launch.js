(function (window, document) {
  ...
  var DEFAULTS = {
    ...
    sessionStorageKey: 'cf_integration_session_id', bootstrapStorageKey: '408farmers_pvx_bootstrap_id_v2',
  ...
  function getBootstrapId(scope) {
    var submissionScope=clean(scope,160),session=safeStorage('sessionStorage');
    if(!submissionScope)return randomId('pvxb_');
    try{
      var stored=JSON.parse(session?.getItem(DEFAULTS.bootstrapStorageKey)||'null');
      if(stored&&stored.scope===submissionScope&&/^pvxb_[A-Za-z0-9_-]{16,80}$/.test(stored.id||''))return stored.id;
    }catch(_){}
    var id=randomId('pvxb_');
    try{session?.setItem(DEFAULTS.bootstrapStorageKey,JSON.stringify({scope:submissionScope,id:id,createdAt:new Date().toISOString()}));}catch(_){}
    return id;
  }
  ...
  function normalizeEntry(entry){var value=clean(entry||window.location.pathname,80).replace(/^\/+|\/+$/g,'').toLowerCase();if(/snapshot|score/.test(value))return'snapshot';if(/buyer/.test(value))return'buyer';if(/auto[_-]?bundle|home[_-]?auto/.test(value))return'home_auto';if(/^auto$|auto_lander/.test(value))return'auto';if(/tech|teacher|healthcare|engineer|professional/.test(value))return'professional';if(/life/.test(value))return'life';if(/business/.test(value))return'business';if(/neighbor/.test(value))return'neighbor';if(/home/.test(value))return'home';return value||'homepage';}
  ...
  function customerSelection(track,entry){return entry==='snapshot'?'coverage_concern':track==='buyer'?'buying_home':track==='renter'?'renter':track==='bundle'?'review_home_auto':track==='auto'?'start_snapshot':'review_owned_home';}
  function discoverySeed(profile){var shopping=clean(profile?.discovery?.answers?.shoppingReason,40)||SHOPPING_REASON[clean(profile?.homeReviewGoal,40)]||'',priority=IMPROVEMENT[clean(profile?.reviewTiming,40)]||'';if(clean(profile?.housingContext,40)==='buyer')shopping='buying_home';return{shoppingReason:shopping,improvementPriorities:priority?[priority]:[],shoppingWords:clean(profile?.reviewContext,240)};}
  ...
  function buildPayload(options) {
    ...
    var payload={
      bootstrap_id:getBootstrapId(extra.submitted_at||profile.createdAt||''),entry_type:config.entry,route_path:window.location.pathname||'/',host_mode:'408farmers',source:attribution.source==='local'?'local':config.source,
      customer_selection:customerSelection(track,config.entry),product_track:track,campaign:config.campaign||attribution.campaign||'direct',campaign_id:attribution.campaign_id||profile.campaignId||'',campaign_variant:attribution.campaign_variant||profile.campaignVariant||'',campaign_zip:attribution.campaign_zip||profile.campaignZip||'',creative:attribution.creative||'',
      ...
      journey_goal:clean(extra.journey_goal,40),review_track:clean(extra.review_track||profile.reviewTrack||track,40),review_reason:clean(extra.review_reason||profile.reviewReason,60),
    ...
    if(track==='buyer'||config.entry==='buyer'){payload.closing_date=clean(profile.closingDate,40);payload.occupancy=clean(profile.occupancy,40);payload.property_zip=clean(profile?.address?.postalCode,10);}
