(function (window, document) {
  ...
  var payoffContact = payoff.querySelector('[data-home-payoff-contact]');
  var payoffContactOptions = payoff.querySelector('[data-home-payoff-contact-options]');
  ...
  if (payoffContact && payoffContactOptions) payoffContact.addEventListener('click', function () {
    payoffContact.hidden = true;
    payoffContactOptions.hidden = false;
    var callbackSlot = payoffContactOptions.querySelector('[data-home-payoff-callback]');
    if (window.CallbackSchedulingContinuity && typeof window.CallbackSchedulingContinuity.mount === 'function') {
      window.CallbackSchedulingContinuity.mount(callbackSlot, {
        form: form,
        productType: form.dataset.cfEntry || 'home',
    ...
    emit('home_snapshot_deferred', Object.assign({ stage: contract.STAGES.PAYOFF }, eventContext(semanticAnswers())));
    announce('Choose a callback time, text Dylan, or call Dylan.');
    var firstChoice = payoffContactOptions.querySelector('button, a');
