(function (window) {
  ...
  function discoveryAnswers(form) {
    ...
    var appointmentReason = { price:'comparison', upcoming_renewal:'renewal_increase', new_home_or_vehicle:'life_change', coverage_concern:'something_else', comparison:'comparison' }[field(form, 'review_reason')] || '';
    if (appointmentReason) reason = appointmentReason;
  ...
  function build(form) {
    ...
    var profile = {
      ...
      reviewTrack: field(form, 'review_track'),
      reviewReason: field(form, 'review_reason'),
      ...
      discovery: {
        schemaVersion: '1.0', productTrack: field(form, 'review_track') || (field(form, 'housing_context') === 'buyer' ? 'buyer' : field(form, 'housing_context') === 'renter' ? 'renter' : field(form, 'home_review_goal') === 'home_auto_bundle' ? 'bundle' : 'home'),
