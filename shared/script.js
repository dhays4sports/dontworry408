...
(() => {
  ...
  const professionalContext = () => ({
    ...
    review_track: String(form.elements.review_track?.value || '').trim(),
    review_reason: String(form.elements.review_reason?.value || '').trim(),
    journey_goal: String(form.elements.journey_goal?.value || '').trim()
  });
  ...
  form.addEventListener('submit', async (event) => {
    ...
    setLeadField('review_reason', String(form.elements.review_reason?.value || ''));
    ...
    const continueToCoverageFit = (leadCaptureStatus = 'confirmed') => {
      ...
      const openDestination = () => {
        ...
        label.textContent=form.elements.journey_goal?.value === 'appointment' ? 'Opening Dylan’s calendar…' : 'Opening CoverageFit…';
        window.CoverageFitLauncher.launch({
    ...
    try {
      ...
      status.textContent = 'We could not confirm that your request was saved. Please retry before choosing an appointment time.';
    }
  });
